...
export async function getData(jid: string, isGroup: boolean = false): Promise<null | object> {
...
export const group = {
  update: async (jid: string, data: Partial<GroupProps>) => {
    return await GroupModels.findOneAndUpdate({ jid }, { ...data }).then((result) => result);
  },
};

export const user = {
  update: async (jid: string, data: Partial<UserProps>) => {
    return await UserModels.findOneAndUpdate({ jid }, { ...data }).then((result) => result);
  },
};

export const registUser = {
  guest: async (data: Partial<UserProps>): Promise<UserProps | null> => {
    return await UserModels.create({ ...data, isRegistered: false }).then((result) => {
      if (result) {
  ...
  auth: async (data: Partial<UserProps>): Promise<UserProps | null> => {
    return await UserModels.create({ ...data, isRegistered: true }).then((result) => {
      if (result) {
...
export const registGroup = {
  guest: async (data: Partial<GroupProps>): Promise<GroupProps | null> => {
    return await GroupModels.create({ ...data, isRegistered: false }).then((result) => {
      if (result) {
        Logger.custom(`Success created guest group ${result}`, "[MONGO]", 183);
        return result;
      } else {
        handleError("Fail created new guest user", data);
        return null;
  ...
  auth: async (data: Partial<GroupProps>): Promise<GroupProps | null> => {
    return await GroupModels.create({ ...data, isRegistered: true }).then((result) => {
      if (result) {
